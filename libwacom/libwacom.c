...
libwacom_get_device(const WacomDeviceDatabase *db,
		    const char *match)
{
...
get_uinput_subsystem(GUdevDevice *device)
{
	...
	while (parent &&
	       !g_udev_device_get_property_as_boolean(parent, "UINPUT_DEVICE")) {
...
get_bus(GUdevDevice *device)
{
	...
	if (parent) {
		if (subsystem &&
		    (g_str_equal(subsystem, "tty") || g_str_equal(subsystem, "serio")))
					  ...
					  const char *path)
{
	...
	for (l = devices; l != NULL; l = l->next) {
		if (!ret &&
		    g_strcmp0(g_udev_device_get_device_file(l->data), path) == 0)
...
static char *
get_device_prop(GUdevDevice *device,
		const char *propname)
{
	char *value = NULL;
		...
		WacomError *error)
{
	...
	*integration_flags =
		WACOM_DEVICE_INTEGRATED_UNSET; // NOLINT: core.EnumCastOutOfRange
	*name = NULL;
	*uniq = NULL;
	bus_str = NULL;
	...
	if (device == NULL) {
		libwacom_error_set(error,
				   WERROR_INVALID_PATH,
				   "Could not find device '%s' in udev",
				   path);
		goto out;
	}
	...
	if (devname != NULL) {
		...
		sysfs_path = g_build_filename("/sys/class/input",
					      devname,
					      "device/properties",
					      NULL);
...
libwacom_copy(const WacomDevice *device)
{
	...
	d->buttons = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_free);
...
static bool
match_is_equal(const WacomMatch *a,
	       const WacomMatch *b)
{
...
static bool
matches_are_equal(const WacomDevice *a,
		  const WacomDevice *b)
{
...
static gboolean
libwacom_same_layouts(const WacomDevice *a,
		      const WacomDevice *b)
{
...
static const WacomDevice *
libwacom_new(const WacomDeviceDatabase *db,
	     const char *name,
	     const char *uniq,
	     int vendor_id,
	     int product_id,
...
builder_is_name_only(const WacomBuilder *builder)
{
	return builder->device_name != NULL && builder->match_name == NULL &&
	       builder->uniq == NULL && builder->vendor_id == 0 &&
	       builder->product_id == 0 && builder->bus == WBUSTYPE_UNKNOWN;
}
 ...
 */
static WacomDevice *
fallback_or_device(const WacomDeviceDatabase *db,
		   const WacomDevice *device,
		   const char *name_override,
		   WacomFallbackFlags fallback_flags)
{
...
static gint
find_named_device(const WacomDevice *device,
		  const char *name)
{
...
static gint
find_uniq_device(const WacomDevice *device,
		 const char *uniq)
{
			  ...
			  WacomError *error)
{
	...
	if (builder_is_name_only(builder)) {
		...
		GList *entry = g_list_find_custom(keys,
						  builder->device_name,
						  (GCompareFunc)find_named_device);
		if (entry)
			device = entry->data;
		ret = fallback_or_device(db, device, builder->device_name, fallback);
		/* Uniq-only behaves like new_from_name but matches on uniq in the match
		 * strings */
	} else if (builder_is_uniq_only(builder)) {
		g_autoptr(GList) keys = g_hash_table_get_values(db->device_ht);
		GList *entry = g_list_find_custom(keys,
						  builder->uniq,
						  (GCompareFunc)find_uniq_device);
		if (entry)
			device = entry->data;
		ret = fallback_or_device(db, device, builder->device_name, fallback);
	} else {
		...
		while (*bus != WBUSTYPE_UNKNOWN) {
			/* Uniq (where it exists) is more reliable than the name which
			 * may be re-used across tablets. So try to find a uniq+name
			 * match first, then uniq-only, then name-only.
			 */
		...
		ret = fallback_or_device(db, device, builder->device_name, fallback);
		if (ret && device != NULL) {
			/* If this isn't the fallback device: for multiple-match
			 * devices, set to the one we requested */
			WacomMatch *used_match = libwacom_match_new(used_match_name,
								    used_match_uniq,
								    *bus,
								    vendor_id,
								    product_id);
...
static void
print_layout_for_device(int fd,
			const WacomDevice *device)
{
...
static void
print_button_evdev_codes(int fd,
			 const WacomDevice *device)
{
	int nbuttons = libwacom_get_num_buttons(device);
	char b;
...
static void
print_buttons_for_device(int fd,
			 const WacomDevice *device)
{
	int nbuttons = libwacom_get_num_buttons(device);
...
static void
print_integrated_flags_for_device(int fd,
				  const WacomDevice *device)
{
	/*
	...
	for (guint i = 0; i < device->matches->len; i++) {
		...
		if (match_is_equal(m, to_remove)) {
			...
			if (match_is_equal(dflt, to_remove)) {
				WacomMatch *first =
					g_array_index(device->matches, WacomMatch *, 0);
				 ...
				 int *num_paired)
{
	...
	if (count > 0)
		memcpy(styli,
		       stylus->paired_styli->data,
		       count * sizeof(WacomStylus *));
	return styli;
}
...
libwacom_stylus_get_num_buttons(const WacomStylus *stylus)
{
	if (stylus->num_buttons == -1) {
		g_warning(
			"Stylus '0x%x' has no number of buttons defined, falling back to 2",
			stylus->id.tool_id);
		return 2;
	}
...
libwacom_stylus_get_type(const WacomStylus *stylus)
{
	if (stylus->type == WSTYLUS_UNKNOWN) {
		g_warning(
			"Stylus '0x%x' has no type defined, falling back to 'General'",
			stylus->id.tool_id);
