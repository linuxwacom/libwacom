		 ...
		 WacomError   *error)
{
	...
	if (g_strv_length (splitted_product) != 4) {
		libwacom_set_error(error, WERROR_UNKNOWN_MODEL, "Unable to parse model identification");
		goto out;
	}
		 ...
		 WacomError            *error)
{
	...
	*name = NULL;
	...
	if (device == NULL) {
		libwacom_set_error(error, WERROR_INVALID_PATH, "Could not find device '%s' in udev", path);
		goto out;
	}
	...
	if (!is_tablet_or_touchpad(device)) {
		...
		if (!parent || !is_tablet_or_touchpad(parent)) {
			libwacom_set_error(error, WERROR_INVALID_PATH, "Device '%s' is not a tablet", path);
			g_object_unref (parent);
			goto out;
		}
		g_object_unref (parent);
	...
	*bus = bus_from_str (bus_str);
	...
	if (*bus == WBUSTYPE_SERIAL) {
		...
		*product_id = 0;
		...
		libwacom_set_error(error, WERROR_UNKNOWN_MODEL, "Unsupported bus '%s'", bus_str);
	}
...
libwacom_new (const WacomDeviceDatabase *db, const char *name, int vendor_id, int product_id, WacomBusType bus, WacomError *error)
{
	...
	if (!db) {
		libwacom_set_error(error, WERROR_INVALID_DB, "db is NULL");
		return NULL;
	}
...
libwacom_new_from_path(const WacomDeviceDatabase *db, const char *path, WacomFallbackFlags fallback, WacomError *error)
{
	...
	switch (fallback) {
		...
		case WFALLBACK_GENERIC:
			break;
		default:
			libwacom_set_error(error, WERROR_BUG_CALLER, "invalid fallback flags");
			return NULL;
	}
	...
	if (!path) {
		libwacom_set_error(error, WERROR_INVALID_PATH, "path is NULL");
		return NULL;
	}
...
bail:
	g_free (name);
	libwacom_set_error(error, WERROR_UNKNOWN_MODEL, NULL);
	return NULL;
}
